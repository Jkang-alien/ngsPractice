...
???
* 이번시간에 말씀드릴 내용은 AF와 VP입니다. 
* NGS 결과를 받아보면 variant에 대한 정보가 많이 있습니다. 
...
* 이번 시간에 첫번째로 Allele frequency에 대해 이야기 해보겠습니다.
...
변이 allele frequency는 63%입니다. sequencing 된 read 중 변이 allele인 T가 63%이고 reference인 G가 37%입니다. 병리의사가 측정한 종양세포 비율은 70%입니다. 
...
실제로는 정확한 종양세포 비율과 allele specific copy number를 알 지 못하기 때문에 단순한 계산으로는 variant alllele frequency만으로 germline과 somatic을 구분하기는 어렵습니다. 좀 더 복잡한 모델링을 통해서는 구분할 수 가 있습니다. 
...
![:scale 70%](ACMG-figure/pedigree.jpg)
???
...
![:scale 60%](ACMG-figure/gnomad.jpg)
???
